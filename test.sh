...
# ユニットテストの作成
try 0 "main(argc, argv)  { return 0; }"
try 42 "main(argc, argv)  { return 42; }"
# ステップ2：加減算のできるコンパイラの作成
try 21 "main(argc, argv) { return 5+20-4; }"
# ステップ3：トークナイザを導入
try 41 "main(argc, argv)  {  return 12 + 34 - 5 ; }"
# 生成規則による演算子の優先順位の表現
try 14 "main(argc, argv)  { return 1 * 2 + 3 * 4; }"
# ステップ4：四則演算のできる言語の作成
try 47 "main(argc, argv)  { return 5+6*7; }"
try 15 "main(argc, argv)  { return 5*(9-6); }"
try 4 "main(argc, argv)  { return (3+5)/2; }"
# ステップ5：単項プラスと単項マイナス
try 3 "main(argc, argv)  { return -5 * 3 - +2 + -5 * -4; }"
# ステップ6: 比較演算子
try 1 "main(argc, argv)  { return 2 + 3 * 1 == 5; }"
try 0 "main(argc, argv)  { return 2 == 3; }"
try 1 "main(argc, argv)  { return 2 != 3; }"
try 0 "main(argc, argv)  { return 3 * 0 != 5 * 0 + 0; }"
try 1 "main(argc, argv)  { return 1 > 0; }"
try 0 "main(argc, argv)  { return 0 > 0; }"
...
try 28 "main(argc, argv)  { return test3(1, 2, 3, 4, 5, 6, 7); }"
ステップ15: 関数の定義に対応する
